...
public class LunaraBiomeProvider extends BiomeProvider {
    private static final Set<Biome> biomes = new HashSet<>();
    private static final double BIOME_SIZE = 32.0d;
    private final LunaraGenerator biomeNoise = new LunaraGenerator();

    public LunaraBiomeProvider(@Nonnull LunaraBiomeProviderSettings genSettings) {
        super(biomes);

        this.biomeNoise.setSeed((int) genSettings.getSeed());
        biomes.add(BiomeRegistries.LUNARA_PLAINS.get());
        biomes.add(BiomeRegistries.LUNARA_FOREST.get());
        biomes.add(BiomeRegistries.LUNARA_MOUNTAINS.get());
    }
