...
public class IslandChunkGenerator extends NoiseChunkGenerator<IslandGenSettings> {
    ...
    public IslandChunkGenerator(IWorld worldIn, BiomeProvider provider, IslandGenSettings settingsIn) {
    ...
    protected double func_222545_a(double p_222545_1_, double p_222545_3_, int p_222545_5_) {
        double d0 = 8.5D;
        double d1 = ((double) p_222545_5_ - (d0 + p_222545_1_ * 8.5D / 8.0D * 4.0D)) * 12.0D * 128.0D / 256.0D / p_222545_3_;
    ...
    protected double[] getBiomeNoiseColumn(int noiseX, int noiseZ) {
        double[] biomeNoiseColumn = new double[2];
        float f = 0.0F;
        float f1 = 0.0F;
        float f2 = 0.0F;
        ...
        biomeNoiseColumn[0] = (double) f1 + this.getNoiseDepthAt(noiseX, noiseZ);
        biomeNoiseColumn[1] = f;

        return biomeNoiseColumn;
    }

    private double getNoiseDepthAt(int noiseX, int noiseZ) {
        double d0 = this.depthNoise.getValue(
                noiseX * 200,
                10.0D,
                noiseZ * 200,
                1.0D,
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(@Nonnull EntityClassification creatureType, @Nonnull BlockPos pos) {
        if (Feature.SWAMP_HUT.func_202383_b(this.world, pos)) {
            ...
            } else if (Feature.OCEAN_MONUMENT.isPositionInStructure(this.world, pos)) {
    ...
    @Override
    public void spawnMobs(@Nonnull ServerWorld worldIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs) {
