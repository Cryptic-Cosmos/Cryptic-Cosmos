...
public class MoonForestBiome extends Biome {
    ...
    public static final TreeFeatureConfig field_230129_h_ = (new TreeFeatureConfig.Builder(new SimpleBlockStateProvider(MOON_LOG), new SimpleBlockStateProvider(MOON_LEAVES), new BlobFoliagePlacer(2, 0))).baseHeight(5).heightRandA(2).foliageHeight(3).ignoreVines().decorators(ImmutableList.of(new BeehiveTreeDecorator(0.002F))).setSapling((net.minecraftforge.common.IPlantable) BlockHandler.MOON_SAPLING.get()).build();
    public static final TreeFeatureConfig FANCY_MOON_TREE_CONFIG = (new TreeFeatureConfig.Builder(new SimpleBlockStateProvider(MOON_LOG), new SimpleBlockStateProvider(MOON_LEAVES), new BlobFoliagePlacer(0, 0))).setSapling((net.minecraftforge.common.IPlantable) BlockHandler.MOON_SAPLING.get()).build();
    ...
    public MoonForestBiome(Builder biomeBuilder) {
        super((new Biome.Builder()).surfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.GRASS_DIRT_GRAVEL_CONFIG).precipitation(Biome.RainType.RAIN).category(Category.FOREST).depth(0.125F).scale(0.05F).temperature(0F).downfall(0.4F).waterColor(16777205).waterFogColor(16777205).parent(null));
